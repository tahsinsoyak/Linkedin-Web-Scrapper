...
driver = webdriver.Chrome(service=service,chrome_options=coptions,options=options)
...
def getProfileURLs(companyName):
    ...
    while page_num > 3:
        time.sleep(1)
        driver.get(f"https://www.linkedin.com/search/results/people/?geoUrn=%5B%22102424322%22%5D&keywords=nima&origin=FACETED_SEARCH&sid=1We&page={page_num}")
        time.sleep(3)
        ...
        # parse the source to extract the profile URLs
        time.sleep(1)
